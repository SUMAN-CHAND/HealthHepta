...
app.post('/doctorsearch', async (req, res) => {
   ...
   `;
        ...
        const doctorResults = await new Promise((resolve, reject) => {
            db.query(query, [input, input, input, input, input, input, input], (err, results) => {
                if (err) {
                    console.error(err);
                    res.status(500).send({ message: 'Error searching for doctors' });
                } else {
...
app.get('/doctortime/:id', async (req, res) => {
    const doctor_id = Number(req.params.id);
    const sql = `SELECT * FROM doctor_regular_time WHERE doctor_id = ${db.escape(doctor_id)}`;
    try {
        db.query(sql, (err, data) => {
            if (err) {
                console.log(err);
    ...
    if (req.session.user) {
        try {
            ...
            if (user.role === 'customer') {
                ...
                const sql1 = `SELECT * FROM doctors_details WHERE id = ${db.escape(doctor_id)}`;
                try {
                ...
                try {
                    const appoimentBook = await new Promise((resolve, reject) => {
                        ...
                        db.query(sql, [doctor_id, appoint_date, appoint_time, name, ph_number, clinic_id, user_id, type_of_visite], (err, result) => {
                            if (err) {
                                console.log(err);
                                return res.status(500).json({ error: 'Failed to book appointment' });
                            } else {
                    ...
                    const commission = await new Promise((resolve, reject) => {
                        const sql1 = "select * from partner_services where service_type = 'Appoiment Book';";
                    ...
                    const addOrderToPartner = await new Promise((resolve, reject) => {
                        const sql2 = "INSERT INTO partner_commision (partner_id, service_type,order_id, commision_type,commision) VALUES (?,'Appoiment Book' ,?, ?,?);";
                        db.query(sql2, [user_id, appoimentBook, commission[0].commision_type, commission[0].commision], (err, result) => {
                            if (err) {
                                reject(err);
                            } else {
    ...
    if (req.session.user) {
        try {
            ...
            if (user.role === 'customer') {
                ...
                const { Test_id, appoint_date, appoint_time, name, ph_number, gender, sample_collection, payment } = req.body[0];
                        ...
                        if (err) {
                            ...
                            try {
                                const labBooking = await new Promise((resolve, reject) => {
                                    db.query(sql, [Test_id, appoint_date, appoint_time, name, ph_number, req.body[1], user_id, gender, sample_collection], async (err, result) => {
                                        if (err) {
                                            console.log(err);
                                            return res.status(500).json({ error: 'Failed to book appointment' });
                                        } else {
                                ...
                                const createPayment = await new Promise((resolve, reject) => {
                                    const sql4 = "INSERT INTO payments (labbooking_id, payment_date,total_amount,payment_status,payment_type,service_type) VALUES (?, ?,?,?,?,'Lab Book');";
                                    db.query(sql4, [labBooking, date, total_amount, 'pending', payment], (err, result) => {
                                        if (err) {
                                            reject(err);
                                        } else {
                                            resolve(result);
                    ...
                    db.query(sql1, [req.body[1]], async (err, data) => {
                        if (err) {
                            ...
                            try {
                                ...
                                // Get the Commission from DB
                                const commission = await new Promise((resolve, reject) => {
                                    const sql1 = "select * from partner_services where service_type = 'Lab Book';";
                                    db.query(sql1, (err, rows) => {
                                        if (err) {
                                            reject(err);
                                        } else {
                                ...
                                // console.log(commission);
                                const addOrderToPartner = await new Promise((resolve, reject) => {
                                    const sql2 = "INSERT INTO partner_commision (partner_id, service_type,order_id, commision_type,commision) VALUES (?,'Lab Book' ,?, ?,?);";
                                    db.query(sql2, [user_id, labBooking, commission[0].commision_type, commission[0].commision], (err, result) => {
                                        if (err) {
                                            reject(err);
                                        } else {
                                            resolve(result.insertId); // Get the newly inserted order ID
...
app.post('/search', async (req, res) => {
    ...
    try {
        ...
        // const user_id = req.session.user.id;
        const query1 = "SELECT  sub_admin.id ,Test_id,Test_Name,Test_Desc,test_imageId,Price FROM address_sub_admin" +
        ...
        if (LabResults.length > 0) {
            ...
            // console.log([LabResults, LabTestimages]);
            // return res.json([productResults, images]);
        }

        return res.json([productResults, images, LabResults, LabTestimages]);
...
app.get('/addtocart/:product_id', (req, res) => {
        ...
        try {
            ...
            if (user.role === 'customer') {
                ...
                const insertOrderItems = await Promise.all(productInfo.map(product => {
                    ...
                    const sql4 = "INSERT INTO order_items (order_id, product_id, quantity) VALUES (?, ?, ?);";
                    return new Promise((resolve, reject) => {
                ...
                const SubAdminIDs = await Promise.all(productInfo.map(product => {
                    const { product_id, quantity } = product;
                    const sql4 = "SELECT sub_admin_id  FROM product_sub_admin where product_id = ?";
                    return new Promise((resolve, reject) => {
                ...
                if (SubAdminIDs.length > 1) {
                    ...
                    const insertOrdersInSubAdmin = await Promise.all(SubAdminIDs.map(SubAdminID => {
                        const sql4 = "INSERT INTO order_sub_Admin (order_id, sub_admin_id) VALUES (?, ?);";
                        return new Promise((resolve, reject) => {
                ...
                // Insert payment method
                const createPayment = await new Promise((resolve, reject) => {
                    const sql4 = "INSERT INTO payments (order_id, payment_date,total_amount,payment_status,payment_type,service_type) VALUES (?, ?,?,?,?,'Medicine Order');";
                ...
                const decreaseQuantity = await Promise.all(productInfo.map(product => {
                    const { product_id, quantity } = product;
                    const sql5 = "UPDATE product SET product_quantity = product_quantity - ? WHERE product_id = ?;";
                    return new Promise((resolve, reject) => {
                ...
                const OrderCountInUserTable = await new Promise((resolve, reject) => {
                    ...
                    const sql2 = "UPDATE user_tbl SET OrderCount = OrderCount + 1 WHERE id = ?;";
                ...
                // Create the order
                const createOrder = await new Promise((resolve, reject) => {
                    const sql2 = "INSERT INTO orders (user_id, order_date, status,role) VALUES (?, ?, 'pending','partner');";
                    db.query(sql2, [user_id, date], (err, result) => {
                        if (err) {
                            reject(err);
                        } else {
                ...
                // Get the Commission from DB
                const commission = await new Promise((resolve, reject) => {
                    const sql1 = "select * from partner_services where service_type = 'Medicine Order';";
                    db.query(sql1, (err, rows) => {
                        if (err) {
                            reject(err);
                        } else {
                ...
                // console.log(commission);
                const addOrderToPartner = await new Promise((resolve, reject) => {
                    const sql2 = "INSERT INTO partner_commision (partner_id, service_type,order_id, commision_type,commision) VALUES (?,'Medicine Order' ,?, ?,?);";
                    db.query(sql2, [user_id, createOrder, commission[0].commision_type, commission[0].commision], (err, result) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result.insertId); // Get the newly inserted order ID
                ...
                // Insert payment method
                const createPayment = await new Promise((resolve, reject) => {
                    const sql4 = "INSERT INTO payments (order_id, payment_date,total_amount,payment_status,payment_type) VALUES (?, ?,?,?,?);";
                    db.query(sql4, [createOrder, date, total_amount, 'pending', payment_type], (err, result) => {
                        if (err) {
                            reject(err);
                        } else {
...
app.patch('/profile/address', async (req, res) => {
    if (req.session.user) {
        ...
        try {
            db.query(sql, [Village, P_O, City, district, State, Pin, user_id], (err, data) => {
...
app.get('/order/:orderId/product/:productId', (req, res) => {
    if (req.session.user) {
        ...
        const sql = "SELECT  product_name , product_price  FROM product where product_id = ?";
...
app.post('/superadmin/generate-coupon', (req, res) => {
    ...
    const sql = "insert into Coupon (`coupon_code`,`discount_percentage`,`expiry_date`,`is_active`,`created_at`) values (?);";
    db.query(sql, [values], (err, data) => {
        if (err) {
            console.log(err)
            return res.json(err);
...
app.get('/superadmin/sales/week', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL + 0 DAY) and payments.payment_status = 'completed';";
        const sql2 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL + 1 DAY) and payments.payment_status = 'completed';";
        const sql3 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE)= DATE_SUB(CURDATE(), INTERVAL + 2 DAY) and payments.payment_status = 'completed';";
        const sql4 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL + 3 DAY) and payments.payment_status = 'completed';";
        const sql5 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL + 4 DAY) and payments.payment_status = 'completed';";
        const sql6 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL + 5 DAY) and payments.payment_status = 'completed';";
        const sql7 = "SELECT  COUNT(*) as no FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL + 6 DAY) and payments.payment_status = 'completed';";
...
app.get('/superadmin/purchase/week', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "SELECT  COUNT(*) as no, sum(product_price*product_quantity) as price FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +0 DAY) and payments.payment_status = 'pending';";
        const sql2 = "SELECT  COUNT(*) as no, sum(product_price*product_quantity) as price FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +1 DAY) and payments.payment_status = 'pending';";
        ...
        const sql4 = "SELECT  COUNT(*) as no, sum(product_price*product_quantity) as price FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +3 DAY) and payments.payment_status = 'pending';";
        const sql5 = "SELECT  COUNT(*) as no, sum(product_price*product_quantity) as price FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +4 DAY) and payments.payment_status = 'pending';";
        const sql6 = "SELECT  COUNT(*) as no, sum(product_price*product_quantity) as price FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +5 DAY) and payments.payment_status = 'pending';";
        const sql7 = "SELECT  COUNT(*) as no, sum(product_price*product_quantity) as price FROM  product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id   where CAST(orders.order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +6 DAY) and payments.payment_status = 'pending';";
...
app.get('/superadmin/productno', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "SELECT COUNT(*) as no, sum(product_price*product_quantity) as price FROM product;";
...
app.get('/superadmin/product/lowstock', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "SELECT COUNT(*) as no, sum(product_price*product_quantity) as price FROM product where product_quantity<=250;";
...
app.get('/superadmin/product/expirying_ptoduct', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "  SELECT COUNT(*) as no, sum(product_price*product_quantity) as price FROM product where expiry < DATE_SUB(CURDATE(), INTERVAL -1 MONTH);";
...
app.get('/superadmin/product/expiry_product', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "SELECT COUNT(*) as no, sum(product_price*product_quantity) as price FROM product where expiry < CURDATE();";
...
app.get('/superadmin/ordersno/week', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +0 DAY);";
        const sql2 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +1 DAY);";
        const sql3 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +2 DAY);";
        const sql4 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +3 DAY);";
        const sql5 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +4 DAY);";
        const sql6 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +5 DAY);";
        const sql7 = "select COUNT(*) as no from Orders where  CAST(order_date AS DATE) = DATE_SUB(CURDATE(), INTERVAL +6 DAY);";
...
app.get('/superadmin/orders', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = " SELECT orders.id,orders.role, product.product_id , user_id,order_date,status,payment_status,payment_type,expected_delivery_date  FROM product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON orders.id = payments.order_id  ";
        db.query(sql1, (err, data) => {
            if (err) {
                return res.json(err);
            }
...
app.get('/superadmin/payments', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = " select payment_id,user_id,order_id,payment_date,total_amount,payment_status,payment_type FROM payments INNER JOIN orders ON payments.order_id = orders.id;";
...
app.patch('/superadmin/updateproduct/:product_id', async (req, res) => {
    ...
    const sql = "UPDATE product SET `product_name` =?,`product_price`=?,`product_quantity`=?,`category`=?,`discount`=? ,`description`=? ,`manufacturing`=?,`expiry`=?,`DrugOrNot`=?where`product_id` = ?;";
    ...
    try {
        db.query(sql, [product_name, product_price, product_quantity, category, discount, description, manufacturing, expiry, dragornot, id,], (err, data) => {
...
app.post('/superadmin/orders/delivery/:orderId', (req, res) => {
    ...
    const sql = 'UPDATE `orders` SET `expected_delivery_date` = ? WHERE `id` = ?;';
    try {
...
app.post('/sub_admin/complete_profile', async (req, res) => {
    ...
    try {
        ...
        // ]
        try {
            db.query(sql, [req.body.LicenceImageId, req.body.SubAdminImageId, req.body.subadmin_id], (err, data) => {
...
app.get('/laboratory/lab_tests', async (req, res) => {
    ...
    try {
        // const user_id = req.session.user.id;
        const query = "select  Test_id, Test_Name ,Test_Desc,Clinic_id,test_imageId,Landmark,Price from Laboratory_Tests_Details INNER JOIN sub_admin ON sub_admin.id = Laboratory_Tests_Details.clinic_id INNER JOIN sub_admin_details ON sub_admin.id = sub_admin_details.sub_admin_id ;";
        const subResults = await new Promise((resolve, reject) => {
            db.query(query, (err, result) => {
                if (err) {
                    console.error('Error retrieving data: ' + err.message);
...
app.get('/book/lab-test/:id', async (req, res) => {
    ...
    try {
        // const user_id = req.session.user.id;
        const query = "select  Test_id, Test_Name ,Test_Desc,Clinic_id,Price,test_imageId,Landmark from Laboratory_Tests_Details INNER JOIN sub_admin ON sub_admin.id = Laboratory_Tests_Details.clinic_id INNER JOIN sub_admin_details ON sub_admin.id = sub_admin_details.sub_admin_id where Test_id = ?;";
        const subResults = await new Promise((resolve, reject) => {
...
app.get('/madicine/medicineshops/:location', (req, res) => {
    ...
    const location = req.params.location;
    const sql = "SELECT  sub_admin.id ,name ,phone ,role, address_sub_admin.address_id ,Village ,P_O,City,district,State,Pin  FROM address_sub_admin INNER JOIN address ON address_sub_admin.address_id = address.address_id  INNER JOIN sub_admin ON sub_admin.id = address_sub_admin.sub_admin_id where address.City = ?";
...
app.post('/sub-admin/home/addproduct', async (req, res) => {
    if (req.session.user) {
        ...
        const addProductIdProduct_Sub_Admin = await new Promise((resolve, reject) => {
            const sql = "Insert into product_sub_admin(`product_id`,`sub_admin_id`)  values(?,?)";
...
app.get('/sub-admin/own/orders', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const sql1 = " SELECT b2b_orders.id, b2b_product.product_id , sub_admin_id,order_date,status,payment_status,payment_type  FROM b2b_product INNER JOIN b2b_order_items ON b2b_product.product_id = b2b_order_items.product_id INNER JOIN b2b_orders ON b2b_orders.id = b2b_order_items.order_id INNER JOIN b2b_payments ON b2b_orders.id = b2b_payments.order_id   where b2b_orders.sub_admin_id = ?";
        db.query(sql1, [user_id], (err, data) => {
            if (err) {
                return res.json(err);
            }
...
app.post("/sub-admin/add-doctor", (req, res) => {
    if (req.session.user) {
        ...
        if (!doc_name || !doc_desc || !specializes || !location || !clinic || !clinic_desc || !doctor_imageId || !Phone_number || !type_of_visite) {
            return res.status(400).json({ error: 'Missing data in the request body' });
        }
    ...
    `;
        ...
        const values = [doc_name, doc_desc, specializes, location, clinic, clinic_desc, clinic_id, doctor_imageId, user_role, Phone_number, type_of_visite];
...
app.post("/sub-admin/add-doctor/time-table", (req, res) => {
    if (req.session.user) {
        ...
        db.query(query, [timeValues.map(time => [doctor_id, time.day_of_week, time.start_time, time.end_time])], (err, result) => {
            if (err) {
                console.error('Error inserting data: ' + err.message);
                return res.status(500).json({ error: 'Error inserting data into the database' });
            } else {
...
app.get('/sub-admin/see-lab-bookings', (req, res) => {
    if (req.session.user) {
        ...
        const clinic_id = user_id;
        const sql = "Select LabTestBookedtable.Test_id,appoint_date,LabTestBookedtable.name,Laboratory_Tests_Details.Test_Name,sample_collection,appoint_time,ph_number,user_id,LabTestBookedtable.clinic_id,ph_number,sub_admin.name as sub_name,LabTestStatus from LabTestBookedtable INNER JOIN Laboratory_Tests_Details ON LabTestBookedtable.Test_id = Laboratory_Tests_Details.Test_id INNER JOIN sub_admin ON sub_admin.id = Laboratory_Tests_Details.clinic_id  where LabTestBookedtable.clinic_id = ?;";
        db.query(sql, [clinic_id], (err, data) => {
            if (err) {
                return res.json("Error");
            }
...
app.get('/user/see-lab-booking', (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        // console.log(user_id)
        // const clinic_id = user_id;
        const sql = "Select labtestbookedtable.id ,LabTestBookedtable.Test_id,appoint_date,LabTestBookedtable.name,Laboratory_Tests_Details.Test_Name,sample_collection,appoint_time,ph_number,user_id,LabTestBookedtable.clinic_id,ph_number,sub_admin.name as sub_name,LabTestStatus from LabTestBookedtable INNER JOIN Laboratory_Tests_Details ON LabTestBookedtable.Test_id = Laboratory_Tests_Details.Test_id INNER JOIN sub_admin ON sub_admin.id = Laboratory_Tests_Details.clinic_id  where LabTestBookedtable.user_id = ?;";
        db.query(sql, [user_id], (err, data) => {
            if (err) {
                console.log(err)
                return res.json("Error");
