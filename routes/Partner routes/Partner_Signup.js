...
router.post('/add-partner', (req, res) => {
    ...
    const checkQuery = 'SELECT * FROM b2c_partner WHERE ph_num = ?';
    ...
    db.query(checkQuery, [ph_num], (checkErr, checkResult) => {
        ...
        const insertQuery = 'INSERT INTO b2c_partner (name, ph_num,permission,password) VALUES (?, ?,?,?)';
...
router.post('/partner/complete_profile', async (req, res) => {
    // console.log('click')
    // console.log(req.body)
    try {  // const sql = "Insert into Sub_Admin (`LicenceImageId`,`SubAdminImageId`) values(?) where id = ?;";
        const sql = "UPDATE b2c_partner SET aadhaar = ?, pan= ?,AadhaarCardImageID=?,PanCardImageID=? WHERE id = ?;";
        ...
        const insertAddressParner = await new Promise((resolve, reject) => {
            const sql1 = "insert into address_partner (address_id,partner_id)values(?,?);";
...
router.post('/partner/login', (req, res) => {
    const sql = "Select * from b2c_partner where `ph_num` = ? ";
